...
export default function Signup() {
  ...
  const getPasswordStrength = () => {
    ...
    const strengths = [
      { label: "Slabă", color: "bg-error" },
      { label: "Slabă", color: "bg-error" },
      { label: "Medie", color: "bg-warning" },
      { label: "Bună", color: "bg-primary" },
      { label: "Puternică", color: "bg-success" },
      { label: "Excelentă", color: "bg-success" },
    ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!agreedToTerms) {
      toast({
        title: "Acceptă termenii",
        description: "Trebuie să accepți Termenii și Politica de Confidențialitate pentru a continua.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (data.user) {
        toast({
          title: "Cont creat!",
          description: "Bine ai venit la Surgical.AI. Verifică-ți email-ul pentru confirmare.",
        });
        navigate("/dashboard");
      }
    } catch (error) {
      ...
      <div className="relative z-10 w-full max-w-md px-4">
        <div className="glass-strong rounded-2xl p-8 border border-border/50 shadow-2xl">
          ...
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold mb-2">Creează-ți contul</h1>
            <p className="text-muted-foreground">Începe călătoria ta spre vizibilitate AI</p>
          </div>
              ...
              {password && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Puterea parolei: <span className="font-medium">{passwordStrength.label}</span>
                  </p>
                </div>
              )}
            </div>
